...
RSpec.describe Api::V1::PersonsController, type: :controller do
  ...
  describe '#update' do
    let!(:person) { create :person, :with_photo }
    let!(:new_name) { 'sasuke' }
    let!(:new_lastname) { 'uchiha' }
    let!(:photo_name) { 'sasuke.jpeg' }
    ...
    context 'When person is not update' do 
    	context 'When person is not found' do
      	...
      	context 'When name has been taken' do
			...
			let!(:person) { create :person, :with_photo }
        	let!(:taken_name) { 'sasuke' }

	        before do 
	          #binding.pry
	          
	         post :create, params: { name: new_name, lastname: new_lastname, photo: new_photo }
	         put :update, params: { id: person.id, name: taken_name }
	        end
